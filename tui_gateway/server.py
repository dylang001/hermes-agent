    ...
    *,
    ...
    if wait_for_peer and skip_reason in {"concurrent_lock", "already_rotated"}:
        ...
        if wait.status == "completed" and wait.messages and wait.session_id:
            ...
            with session["history_lock"]:
                # Adopt winner tip regardless of history_version — the
                # concurrent path must not leave the live UI on the parent.
                session["history"] = list(wait.messages)
                session["history_version"] = int(session.get("history_version", 0)) + 1
            usage = _get_usage(agent)
            return max(0, len(history) - len(wait.messages)), usage

        # Timed out / failed — surface explicit skip reason; do not mutate history.
        try:
            agent._last_compress_skip_reason = (
                "peer_timeout" if wait.status == "timeout" else "peer_failed"
            )
    ...
    with session["history_lock"]:
        if skip_reason in {"concurrent_lock", "already_rotated"}:
            # wait_for_peer disabled — still refuse to clobber history.
            usage = _get_usage(agent)
            return 0, usage

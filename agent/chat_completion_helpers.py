...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    # falling back to it loops the failure. Identity semantics are owned by
    # agent.backend_identity. A same-provider/model entry with an explicit
    # credential is the one exception: it is an intentional key rotation,
    ...
    if (
        should_skip_candidate(fb_ident, current_ident)
        and not (same_provider_model and explicit_credential_fallback)
    ):
        logger.warning(
